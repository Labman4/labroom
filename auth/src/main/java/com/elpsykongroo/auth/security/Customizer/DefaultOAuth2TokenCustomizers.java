    ...
    public void customize(OAuth2TokenContext tokenContext) {
        ...
        if (!CollectionUtils.isEmpty(cnfClaims)) {
            if (tokenContext instanceof JwtEncodingContext) {
                JwtEncodingContext context = (JwtEncodingContext) tokenContext;
                context.getClaims().claim("cnf", cnfClaims);
            } else {
                OAuth2TokenClaimsContext context = (OAuth2TokenClaimsContext) tokenContext;
                context.getClaims().claim("cnf", cnfClaims);
            }
        }
        ...
        if (tokenContext
            ...
            Map<String, Object> currentClaims = null;
            if (tokenContext instanceof JwtEncodingContext) {
                JwtEncodingContext context = (JwtEncodingContext) tokenContext;
                currentClaims = context.getClaims().build().getClaims();
            } else {
                OAuth2TokenClaimsContext context = (OAuth2TokenClaimsContext) tokenContext;
                currentClaims = context.getClaims().build().getClaims();
            }
